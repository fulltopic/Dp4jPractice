...
import rl.dqn.reinforcement.dqn.MessageParseUtils
...
class TenhouXmlFileReader(fileName: String) {
  ...
  def parseInit(node: Node): Unit = {
    ...
    currentStats(oya)(PeerStateLenPerPlayer * oya + OyaOffset) = 1
...
//    val currState = currentStats(index).clone()
  ...
  def parseMinkan(index: Int, m: Int): Unit = {
    ...
    currentStats(index)(kanTile / NumPerTile) += MessageParseUtils.kanValue
    ...
    updateBoard(kanTiles.tail, index) //had one tile dropped before minkan
  }
  ...
  def parseAnkan(index: Int, m: Int): Unit = {
    ...
    currentStats(index)(ankanTile / NumPerTile) += MessageParseUtils.kanValue
  ...
  def parseKakan(index: Int, m: Int): Unit = {
    ...
    currentStats(index)(kakanTile / NumPerTile) += MessageParseUtils.kanValue
  ...
  def parsePong(index: Int, m: Int): Unit = {
    ...
    currentStats(index)(pongTile / NumPerTile) += MessageParseUtils.pongValue
...
//  }
  ...
  def parseReach(node: Node): Unit = {
    ...
    step match {
      case 1 =>
        currentStats(who)(ReachOffset) = MessageParseUtils.reachValue

        createDataPair(REACHWoAccept, who)

        for (playIndex <- 0 until PlayerNum) {
          val index = ((who - playIndex + PlayerNum) % PlayerNum + 1) * PeerStateLenPerPlayer + ReachOffset
          currentStats(playIndex)(index) = ReachStep1
        }
      ...
      case 2 =>
        ...
        for (playIndex <- 0 until PlayerNum) {
          val index = ((who - playIndex + PlayerNum) % PlayerNum + 1) * PeerStateLenPerPlayer + ReachOffset
          currentStats(playIndex)(index) = ReachStep2
        }
  ...
  def updateBoard(tile: Int, index: Int): Unit = {
    for (playerIndex <- 0 until PlayerNum) {
      val tileIndex = (((index - playerIndex + PlayerNum) % PlayerNum) + 1) * PeerStateLenPerPlayer + (tile / NumPerTile)
      currentStats(playerIndex)(tileIndex) += 1
    }
  }

  def updateBoard(tiles: Array[Int], index: Int): Unit = {
      tiles.foreach(tile => updateBoard(tile, index))
  }
