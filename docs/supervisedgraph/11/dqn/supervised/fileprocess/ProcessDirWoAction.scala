...
import rl.dqn.supervised.fileprocess.withpeers.{FileProcessorWithPeers, FileProcessorWoAccept}
import rl.dqn.supervised.utils.ReplayDB

class ProcessDirWoAction(dirPath: String, objFileDirPath: String, fileNum: Int, numPerObj: Int) {
  private val logger = Logger("ProcessDirWoAction")
  ...
  def processFile(file: File): Unit = {
    ...
    val processor = new FileProcessorWoAccept(file.getAbsolutePath)
    processor.readFile()
