...
//TODO: Is sparse matrix input better?
class FileProcessorWithPeers(fileName: String){
  private val logger = Logger("FileProcessorWithPeers")
...
//      logger.info("--------------------------> Interrupted game, give up")
  ...
  def parseKita(index: Int, m: Int): Unit = {
    ...
    logger.info("----------------------------------------> Kita has not been implemented " + currentStats(PlayerNum)(0))
  }
...
//    updateBoard(tiles) // Ankan no shown
    ...
    createTransition(origState, currentStats(index), AnKan, DefaultReward, false)
  }
...
//    updateBoard(tiles.tail)  //TODO: Should ?
    ...
    createTransition(origState, currentStats(index), KaKan, DefaultReward, false)
  }
  ...
  def parsePong(index: Int, m: Int): Unit = {
    ...
    createTransition(origState, currentStats(index), Pong, DefaultReward, false)
  }
  ...
  def parseChow(index: Int, m: Int): Unit = {
    ...
    createTransition(origState, currentStats(index), Chow, DefaultReward, false)
  }
  ...
  def parseReach(node: Node): Unit = {
    ...
    step match {
      case 1 =>
        ...
        createTransition(origState, currentStats(who), REACH, ReachReward, false)
